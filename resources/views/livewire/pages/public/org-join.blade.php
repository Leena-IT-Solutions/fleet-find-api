...
new class extends Component
{
    ...
    {
        // $organization can be passed as Model or ID. Volt route model binding works.
        $this->organization = $organization;
    }

    public function rendering($view)
    {
        // Public pages do not use layouts.app (which has sidebar/auth stuff).
        // Let's use layouts.guest if it exists, or just custom guest layout.
        // Let's check layouts folder first to see what's available.
        $view->layout('layouts.guest');
    }
}; ?>

<div class="min-h-screen flex flex-col justify-between bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
    <div class="sm:mx-auto sm:w-full sm:max-w-md my-auto flex flex-col gap-6">
        <!-- Logo / Brand -->
        <div class="flex flex-col items-center gap-2">
            <x-application-logo class="w-16 h-16 logo-spin" />
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">FleetFind</h1>
            <p class="text-xs font-semibold uppercase tracking-widest text-indigo-600">Smart Route Tracking</p>
        </div>

        <!-- Onboarding Main Card -->
        <div class="bg-white border border-slate-200/80 shadow-xl rounded-2xl p-8 flex flex-col gap-6 text-center">
            ...
            <div class="bg-indigo-50/50 border border-indigo-100 rounded-xl p-4 flex flex-col gap-2 items-center">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs font-bold text-indigo-750">Opening FleetFind mobile app...</span>
                </div>
            ...
            <div class="border-t border-slate-100 pt-6 flex flex-col gap-4 text-left">
                ...
                <ol class="flex flex-col gap-3">
                    <li class="flex gap-3 items-start">
                        <span class="w-5 h-5 rounded-full bg-slate-100 text-slate-650 text-xs font-bold flex items-center justify-center shrink-0">1</span>
                        <div class="text-xs text-slate-600">
                            <strong>Install FleetFind</strong> app on your Android or iOS device using the links above.
                        </div>
                    </li>
                    <li class="flex gap-3 items-start">
                        <span class="w-5 h-5 rounded-full bg-slate-100 text-slate-650 text-xs font-bold flex items-center justify-center shrink-0">2</span>
                        <div class="text-xs text-slate-600">
                            <strong>Register as a Parent</strong> within the mobile app.
                        </div>
                    </li>
                    <li class="flex gap-3 items-start">
                        <span class="w-5 h-5 rounded-full bg-slate-100 text-slate-650 text-xs font-bold flex items-center justify-center shrink-0">3</span>
                        <div class="text-xs text-slate-600">
                            Once logged in, you'll be automatically routed to and synced with <strong>{{ $organization->name }}</strong>.
                        </div>
                    </li>
                </ol>
            </div>
    ...
    <!-- Footer -->
    <div class="sm:mx-auto sm:w-full sm:max-w-md text-center text-[10px] text-slate-400">
        &copy; {{ date('Y') }} FleetFind. All rights reserved.
    </div>

    <!-- Script to execute deep link redirect -->
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Optional fallback: if the page is still active after a timeout, 
            // the app is likely not installed, so the user stays on the page.
            setTimeout(function() {
                console.log("Deep link redirect timeout. Stay on the download fallback page.");
            }, 2500);
        });
    </script>
</div>
